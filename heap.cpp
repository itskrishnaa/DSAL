...
public:
    void insert(string name, int priority) {
        if (size >= MAX) return void(cout << "Queue is full!\n");
        heap[size++] = {name, priority};
        heapifyUp(size - 1);
    }

    void extractMax() {
        if (size == 0) return void(cout << "Queue is empty!\n");
    ...
    void display() {
        ...
        for (int i = 0; i < size; i++)
            cout << "Name: " << heap[i].name << " | Priority: " 
                 << (heap[i].priority == 10 ? "Serious" : heap[i].priority == 5 ? "Non-serious" : "Checkup") << endl;
    }
};
...
int main() {
    ...
    do {
        ...
        cin >> choice;
        switch (choice) {
            case 1: cout << "Name: "; cin >> name; cout << "Priority (10/5/1): "; cin >> priority; pq.insert(name, priority); break;
            case 2: pq.extractMax(); break;
